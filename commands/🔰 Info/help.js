...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    if (args[0]) {
      ...
      let button_tutorial = new MessageButton().setStyle('LINK').setEmoji("933958931884175370").setLabel("Invite").setURL("https://discord.com/api/oauth2/authorize?client_id=909383364555988993&permissions=0&scope=bot")
      let menuOptions = [{
          label: "Overview",
          value: "Overview",
          emoji: "933958931884175370",
      ...
      let OverviewEmbed = new MessageEmbed()
        .setColor("#e80dfd").setThumbnail(es.thumb ? es.footericon : null)
...
<a:pcdev:930585574002266172> Support [**Premier Crafty**](https://discord.gg/kyGRVPh4xs)`)
    ...
    function allotherembeds_eachcategory(filterdisabled = false) {
      ...
      var embed0 = new MessageEmbed()
        ...
        .addField(`<:prplearrow:933924450502594560> **Server Related Commands**`, ">>> " + client.commands.filter((cmd) => cmd.category === "🔰 Info" && cmd.type === "server").sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
        .addField(`<:prplearrow:933924450502594560> **Bot Related Commands**`, ">>> " + client.commands.filter((cmd) => cmd.category === "🔰 Info" && cmd.type === "bot").sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
      embeds.push(embed0)
      ...
      var embed3 = new MessageEmbed()
        ...
        .addField("<:prplearrow:933924450502594560> **Song Commands**", "> " + client.commands.filter((cmd) => cmd.category === "🎶 Music" && cmd.type?.includes("song")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
        .addField("<:prplearrow:933924450502594560> **Bot Commands**", "> " + client.commands.filter((cmd) => cmd.category === "🎶 Music" && cmd.type?.includes("bot")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
      embeds.push(embed3)
      ...
      var embed5 = new MessageEmbed()
        ...
        .addField("\u200b", "__**Sub-Categorized Commands:**__")
        .addField("<:prplearrow:933924450502594560> **Bot Related Commands**", "> " + client.commands.filter((cmd) => cmd.category === "⚙️ Settings" && cmd.type?.includes("bot")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
      ...
      var embed9 = new MessageEmbed()
        ...
        .addField("\u200b", "__**Sub-Categorized Commands:**__")
        .addField("<:prplearrow:933924450502594560> **Information & Manage**", "> " + client.commands.filter((cmd) => cmd.category === "👑 Owner" && cmd.type?.includes("info")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
        .addField("<:prplearrow:933924450502594560> **Adjust the Bot**", "> " + client.commands.filter((cmd) => cmd.category === "👑 Owner" && cmd.type?.includes("bot")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
      embeds.push(embed9)
