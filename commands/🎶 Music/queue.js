    ...
    if (!tracks.length)
      return message.reply({
        embeds: [new MessageEmbed()
          ...
          .setColor("#FFD700").addField(eval(client.la[ls]["cmds"]["music"]["queue"]["variablex_1"]), eval(client.la[ls]["cmds"]["music"]["queue"]["variable1"]))
    ...
    if (tracks.length < 15)
      return message.reply({
        embeds: [new MessageEmbed()
          ...
          .setColor("#FFD700").setDescription(tracks.map((track, index) => `**\` ${++index}. \` [${track.title.substr(0, 60).replace(/\[/igu, "\\[").replace(/\]/igu, "\\]")}](${track.uri})** - \`${track.isStream ? `LIVE STREAM` : format(track.duration).split(` | `)[0]}\`\n> *Requested by: __${track.requester.tag}__*`).join(`\n`))
        ]
      }).then(msg => {
        setTimeout(() => {
          try {
