      ...
      try {
        ...
        if (playercreated.has(player.guild)) {
          ...
          if (!player.get("autoresume") && channel && channel.permissionsFor(guild.me).has(Permissions.FLAGS.SEND_MESSAGES)) {
            channel.send({
              embeds: [
                new MessageEmbed().setColor("#FFD700")
            ...
            if (i.customId == `8`) {
              ...
              i.reply({
                embeds: [new MessageEmbed()
                  ...
                  .setTitle(`🔊 Volume set to ${amount}`)

                ]
              })
              if(amount >= 150) return i.reply({ content: `Cannot higher the player volume further more.`});
...
function generateQueueEmbed(client, player, track) {
  ...
  embed.addField(`Duration: `, `\`${format(player.queue.current.duration).split(" | ")[0]}\``, true)
  embed.addField(`Song By: `, `\`${player.queue.current.author}\``, true)
  ...
  let skip = new MessageButton().setStyle('SECONDARY').setCustomId('1').setEmoji(`⏭`)
  let stop = new MessageButton().setStyle('SECONDARY').setCustomId('2').setEmoji(`⏹`)
  let pause = new MessageButton().setStyle('SECONDARY').setCustomId('3').setEmoji('⏸')
  let autoplay = new MessageButton().setStyle('SECONDARY').setCustomId('4').setEmoji('🔁')
  let shuffle = new MessageButton().setStyle('SECONDARY').setCustomId('5').setEmoji('🔀')
  if (!player.playing) {
    pause = pause.setStyle('SECONDARY').setEmoji('▶️')
  }
  if (player.get(`autoplay`)) {
    autoplay = autoplay.setStyle('SECONDARY')
  }
  let songloop = new MessageButton().setStyle('SECONDARY').setCustomId('6').setEmoji(`🔁`)
  let queueloop = new MessageButton().setStyle('SECONDARY').setCustomId('7').setEmoji(`🔂`)
  let vup = new MessageButton().setStyle('SECONDARY').setCustomId('8').setEmoji('🔊')
  let vdown = new MessageButton().setStyle('SECONDARY').setCustomId('9').setEmoji('🔉')
  let forwardbutton = new MessageButton().setStyle('SECONDARY').setCustomId('Forward').setEmoji('⏩')
  ...
  const row2 = new MessageActionRow().addComponents([songloop, queueloop, vdown, vup, forwardbutton]);
  return {
    embeds: [embed],
    components: [row, row2]
  }
